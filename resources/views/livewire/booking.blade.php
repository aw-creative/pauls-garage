...
@endif
    ...
    <input type="text" wire:model.lazy='customer.name' class="ml-2 @if($emailvalid) border-green-400 @endif ">
    </label>
    <label> Email Address
        @error('customer.email'))<span class="text-red">{{$message}}</span>@enderror
    <input type="email" wire:model.lazy='customer.email' class="ml-2 @if($emailvalid) border-green-400 @endif ">
    </label>
    <label> Phone Number
        @error('customer.phone'))<span class="text-red">{{$message}}}}</span>@enderror
        <input type="text" wire:model.lazy='customer.phone' wire:change="validateEmail" class="ml-2 @if($emailvalid) border-green-400 @endif ">
    </label>
    <select wire:model="bookingStart">
    ...
    <div wire:loading>Confirming Your Bokking</div>
</form>
@else
