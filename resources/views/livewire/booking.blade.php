...
<div class="w-full my-4">
    ...
    @foreach($futuredates as $date)
        <option @if(in_array(Carbon\Carbon::parse($date),$bookinglist->toArray())) disabled @endif >{{Carbon\Carbon::parse($date)->format('d-m-y')}}</option>
    @endforeach
    </select>
</div>
    @if($bookingStart)
    ...
    @foreach($timeslots as $date)
        <option @if(in_array(Carbon\Carbon::parse($date),$bookinglist->toArray())) disabled @endif >{{Carbon\Carbon::parse($date)->format('H:i')}}</option>
    @endforeach
    </select>
</div>
    @endif
</div>
    <button class="rounded-md p-4 bg-green-500 hover:bg-green-400 w-full mt-6"><div wire:loading wire:target="savebooking">
        Creating Booking</div> Make Booking </button>
</form>
@else
